// Function A3E_fnc_GetPlayers removed by Fred (Hope this leady not into trouble.)
drn_fnc_Escape_OnSpawnGeneralSoldierUnit = {
    ...
    /*Changed by Fred: Not every soldier shall have a radio*/
    if ("ItemRadio" in (assignedItems _this)) then {
		if (random 100 > 20) then {
	...
	/*Changed by Fred: Add some advanced medic suff*/
	
    if (random 100 < 20) then {
        _this addItem "ACE_elasticBandage";
    };
    if (random 100 < 20) then {
        _this addItem "ACE_packingBandage";
    };
    if (random 100 < 20) then {
        _this addItem "ACE_fieldDressing";
    };
    if (random 100 < 20) then {
        _this addItem "ACE_quikclot";
    };
    if (random 100 < 20) then {
...
//Changed by Fred, use the common getPlayer function
    private ["_group"];    
    _group = [] call A3E_fnc_getPlayerGroup;    
    _group
};
