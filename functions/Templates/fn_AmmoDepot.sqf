    ...
    _instanceNo = drn_BuildAmmoDepot_MarkerInstanceNo;
     ...
     /* Start changed by Fred: The position is already marked on map this point shall not glow like a light house*/
     /*
    ...
    _object setDir 20;
    */
    /* End changed by Fred: The position is already marked on map this point shall not glow like a light house*/
    ...
    if (_car != "") then {
        _randomNo = random 4;
	/* Changed by Fred: increase distance because of the camo net :*/
        _pos = [(_middlePos select 0) + 10, (_middlePos select 1), 0];
        _angle = 45;
	...
	/* Changed by Fred: increase distance because of the camo net :*/
        ...
        if (_randomNo > 3) then {
	/* Changed by Fred: increase distance because of the camo net :*/
            _pos = [(_middlePos select 0), (_middlePos select 1) + 10, 0];
    ...
    /* Changed by Fred: add camo net :*/
    _pos = [(_middlePos select 0), (_middlePos select 1), 0];    
    _object = createVehicle ["CamoNet_BLUFOR_big_Curator_F", _pos, [], 0, "NONE"];
    _object setPos _pos;
    _object setDir 90;
