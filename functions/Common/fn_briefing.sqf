...
if(isserver) then {
	...
	_trigger setTriggerArea[0, 0, 0, false];
	_trigger setTriggerActivation["NONE", "PRESENT", false];
	_trigger setTriggerTimeout [60, 60, 60, true]; // Changed by Fred: It could be that players awake 
	
        /* Changed by Fred: ACE support*/
	//@ToDo: Zeus slot shall be marked as dead, otherwise the game will not end. Furthermore all other triggers must be checkted.
	_trigger setTriggerStatements["isServer && isMultiplayer && A3E_EscapeHasStarted && ((({(_x getVariable [""A3E_isDead"",false])} count playableUnits) + ({(_x getVariable ""ACE_isUnconscious"")} count playableUnits)) == count playableUnits )", "a3e_var_Escape_AllPlayersDead = true;publicVariable ""a3e_var_Escape_AllPlayersDead"";[] spawn A3E_FNC_FailTasks;", ""];
};
