...
waitUntil  {
    diag_log format["wait for valid player group", _x];
	_playerGroup = [] call A3E_fnc_GetPlayerGroup;
...
/* Start changes by Fred: other start setup, with a prision guard */
/*
...
*/
	startMarker = createMarker ["respawn ", _guardPos];
    startMarker setMarkerSize [0,0];
    A3E_PrissonGuard = createVehicle [a3e_arr_Escape_PrisonGuardType, _startPos, [], 0, "CAN_COLLIDE"];
    A3E_PrissonGuard unassignItem "ItemMap";
	A3E_PrissonGuard removeItem "ItemMap";
	A3E_PrissonGuard unassignItem "ItemCompass";
	A3E_PrissonGuard removeItem "ItemCompass";
	A3E_PrissonGuard unassignItem "itemGPS";
	A3E_PrissonGuard removeItem "itemGPS";
	A3E_PrissonGuard unassignItem "NVGoggles";
	A3E_PrissonGuard removeItem "NVGoggles";
	A3E_PrissonGuard unassignitem "ItemRadio";
	A3E_PrissonGuard removeItem "ItemRadio";                       
                                    	
	_itemList = [];
    for "_i" from 0 to (count a3e_startMedicBackpackItems - 1) do {
        private ["_item", "_itemClassName", "_probabilityOfPrecence", "_minCount", "_maxCount"];
        
        _item = a3e_startMedicBackpackItems select _i;
       ...
       if (random 100 <= _probabilityOfPrecence) then {
           ...
           _itemList pushBack [_itemClassName, _itemCount];
	   };
	};   
    
    if (count _itemList > 0) then {
        A3E_PrissonGuard addBackpackGlobal "B_AssaultPack_khk";        
        {
            private ["_itemName","_numberOfItems"];           
            _itemName = _x select 0;
            _numberOfItems = _x select 1;
            
            for "_i" from 0 to _numberOfItems do {
                diag_log format["Escape debug: (fn_initServer.sqf) add %1 to A3E_PrissonGuard", _x]; 
            	A3E_PrissonGuard addItemToBackpack _itemName;
            };            
        } foreach _itemList;
    };
                
	// rais a public event for all players that this unit was created.
	publicvariable "A3E_PrissonGuard";

	/* End changes by Fred: other start setup, with a prision guard */
